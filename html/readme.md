...
html在尖括号内部（即标签开始块或结束块）是一个环境，在尖括号外部是另外一个环境。前者单词间的空格是可随意添加的，类似于C源码的词法环境；后者可视为一整块“字符块”，每个字符都是不可忽略的，其效果参考`pre`环境下展示。分析之后，认为html中，单词分以下几种，
...
3. 以字母开头,表示属性的名字，或者属性的值

4. 以'>'开头，表示StringElement对象，这个比较特殊，是游离于各Tag之间的对象。

5. 以单引号或者双引号开始的表示属性的值。
